...
class RobotState:
    ...
    def __init__(self):
        ...
        ------
        pub_ont: 
           It publishes on 'state/new_ontology' topic
        pub_battery: i
           It publishes on 'state/battery_low' topic
        Parameters:
        ------
        ont_needed : bool
           Define if a new ontology is needed to be loaded (initialized as True).
        battery_low : bool
           Define if battery level of the robot is low (initialized as True).
       
        Two threads are started and each one refers to a specific publisher that publishes
        messages on the corresponding topic.
        """   
        # Initialise this node.
        rospy.init_node(pnm.NODE_ROBOT_STATE, log_level=rospy.INFO)
        # Initialise ontology state
        self._ont_needed = True
        pub_ont = rospy.Publisher(pnm.TOPIC_LOAD_ONTOLOGY, Bool, queue_size=1, latch=True)
        pub_ont.publish(Bool(self._ont_needed))
        if self._ont_needed:
    ...
    def _is_battery_low(self):
        ...
        self._battery_notifier(pub_battery)
        
    def _battery_notifier(self, pub_battery):
        """
        ...
        ------
        pub_battery:
          Publisher to the 'state/battery_low' topic
        Parameters:
        ------
        delay: float
          Simulate time needed to recharge the battery and th usage time.
    ...
    def _print_info(self, msg):
         ...
         ------
         msg: str
         It containes the message to print on terminal.
         """
